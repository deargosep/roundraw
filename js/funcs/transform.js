function transform(wb, _view) {
    ...
    for (var i = 0; i < wb.length; i++) {
        ...
        for (var k = 0; k < seg.length; k++) {
            ...
            if (typeof seg[k][seg[k].length-1] == 'number')  {
                seg_[k].push([0]);
                seg[k].push([0]);
            }
            seg_[k][seg_[k].length-1][0] = seg[k][seg[k].length-1][0];
            if (!wb[i].z) wb[i].z = 0;
            //degX
            ...
            var zx = -(seg_[k][0] + wb[i].x) * Math.sin(degX)  + (seg_[k][seg_[k].length-1][0] + wb[i].z) * Math.cos(degX);

            var y1 = (seg_[k][1] + wb[i].y) * Math.cos(degY) + (0) * Math.sin(degY);
            ...
            var dis = (view_.deX) ? -innerHeight : innerHeight ;
...
seg_[k][seg_[k].length-1][0] = zx - wb[i].z;
