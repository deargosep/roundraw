brush.cursor = [
    ...
    function (x, y) {
      ...
      //  tctx.clearRect(0, 0, (innerWidth * devicePixelRatio), (innerHeight * devicePixelRatio));
        ...
        tctx.beginPath();
        tctx.arc(x, y, 10 * devicePixelRatio, 0, 2 * Math.PI, false);
        tctx.stroke();
        tctx.fill();
        }
        loop1: for (var i = erange1; i < erange2; i++) {
            loop2: for (var k = 1; k < wb[i].segments.length; k++) {
                if (doesLineInterceptCircle([wb[i].x + wb[i].segments[k][0] + centerw, wb[i].y + wb[i].segments[k][1] + centerh], [wb[i].x + wb[i].segments[k - 1][0] + centerw, wb[i].y + wb[i].segments[k - 1][1] + centerh], [x, y], 10 * devicePixelRatio + wb[i].strokeWidth / 2)) {
                   /* tctx.fillStyle = 'red';
                    ...
                    */
                    ...
                    loop3: for (var l = 0; l < wb[i].segments.length; l++) {
                        ...
                        tctx.strokeStyle = 'rgba(255,255,255,0.5)';
                        tctx.beginPath();
                        tctx.arc(wb[i].x + wb[i].segments[l][0] + centerw, wb[i].y + wb[i].segments[l][1] + centerh, 10 * devicePixelRatio, 0, 2 * Math.PI, false);
                        tctx.stroke();
                        tctx.fill();
                    }
                    break loop1;
    ...
    if (readings.selected >= 0) {
        ...
        for (var k = 0; k < wb[i].segments.length; k++) {
            if (Math.sqrt(Math.pow((wb[i].x + wb[i].segments[k][0] + centerw) - x, 2) + Math.pow((wb[i].y + wb[i].segments[k][1] + centerh) - y, 2)) < 10 * devicePixelRatio) {
                ...
                tctx.beginPath();
                tctx.arc(wb[i].x + wb[i].segments[k][0] + centerw, wb[i].y + wb[i].segments[k][1] + centerh, 10 * devicePixelRatio, 0, 2 * Math.PI, false);
                tctx.stroke();
                tctx.fill(); 
                readings.selPoint = k;
            }
    ...
    function (x, y) {
        ...
        loop1: for (var i = erange1; i < erange2; i++) {
            loop2: for (var k = 0; k < wb[i].segments.length; k++) {
                if ((k > 0 || wb[i].closed) && doesLineInterceptCircle([wb[i].x + wb[i].segments[k][0] + centerw, wb[i].y + wb[i].segments[k][1] + centerh], [wb[i].x + wb[i].segments[(k == 0) ? wb[i].segments.length-1 : k - 1][0] + centerw, wb[i].y + wb[i].segments[(k == 0) ? wb[i].segments.length-1 : k - 1][1] + centerh], [x, y], 10 * devicePixelRatio + wb[i].strokeWidth / 2)) {
                   /* tctx.fillStyle = 'red';
                    socket.send('del&' + wb[i].id);
                    removed.push(view.wb[i]);
                    view.wb.splice(i, 1);
